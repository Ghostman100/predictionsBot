        ...
        if bet['raw_data']['status'] == 'success'
          ...
          if bet['id'] == id
            if bet['bet']['is_winning_bet'] == true
              ...
              ws.capture "http://116.203.50.7:3000/image?value=#{bet['bet']['amount'].to_i}&bet=#{bet['bet']['coef']}&1st_team=#{bet['raw_data']['radiant_team_name']}&2nd_team=#{bet['raw_data']['dire_team_name']}&winner=#{bet['bet']['team']}&map=#{bet['raw_data']['map'][-1]}&time=#{bet['bet']['created_at']}", "image.png", width: 1024, height: 350
